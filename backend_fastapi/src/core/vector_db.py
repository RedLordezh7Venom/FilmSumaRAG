...
def add_movie_vectors(movie_id: str, chunks: List[str], vectors: np.ndarray) -> None:
    """
    Add movie vectors to ChromaDB.
    We associate each chunk with the movie_id via metadata.
    """
    count = len(chunks)
    
    # Generate unique IDs for each chunk
    ids = [f"{movie_id}_{i}" for i in range(count)]
    
    # Prepare metadata
    metadatas = [{"movie_id": movie_id, "chunk_index": i} for i in range(count)]
    ...
    print(f"✅ Upserted {count} chunks for movie_id: {movie_id} to ChromaDB")

def search_movie(movie_id: str, query_vector: np.ndarray, n_results: int = 3) -> List[str]:
    """
    ...
    results = collection.query(
        ...
        n_results=n_results,
        where={"movie_id": {"$eq": movie_id}}  # Filter by movie_id
    )
...
def has_movie(movie_id: str) -> bool:
    """
    Check if a movie exists in the database.
    We do a lightweight query for 1 item with this metadata.
    """
    result = collection.get(
        where={"movie_id": {"$eq": movie_id}},
        limit=1
    )
    return len(result['ids']) > 0

def get_movie_documents(movie_id: str) -> List[str]:
    """
